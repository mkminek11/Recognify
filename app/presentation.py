...
def extract_images(presentation_file: FileStorage, draft_id: int) -> bool:
    ...
    for slide_n, slide in enumerate(pres.slides):
        for shape in slide.shapes:
            if shape.shape_type == SHAPE_TYPE.PICTURE:
                ...
                draft_img = DraftImage(draft_id, filename, pres_n, slide_n + 1, label = "")
                db.session.add(draft_img)
                image_n += 1
            elif shape.has_text_frame:
                text = getattr(shape, "text", "")
                if not text: continue
                save_labels(text, slide_n, draft_id)
    db.session.commit()
...
def save_labels(text: str, slide: int, draft_id: int) -> bool:
    if not text: return False
    labels = [line.strip() for line in text.splitlines() if line.strip()]
    for label in labels:
        draft_label = DraftLabel()
        draft_label.draft_id = draft_id
        draft_label.label = label
        draft_label.slide = slide
